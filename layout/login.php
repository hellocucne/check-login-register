...
<div id="login" class="d-none">
    ...
    <div class="card px-4 py-5" style="width: 25rem;">
        ...
        <div class="card-body">
            ...
            <form class="mt-3 row" action="backend/auth.php" method="POST">
                <div class="mb-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const registerForm = document.querySelector('#login form.d-none');
        ...
        // Add event listener to switch between login and register forms
        const loginLink = document.querySelector('.login a.clicked');
        const registerLink = document.querySelector('.login a:not(.clicked)');

        loginLink.addEventListener('click', function () {
            loginLink.classList.add('clicked');
            registerLink.classList.remove('clicked');
        ...
        registerLink.addEventListener('click', function () {
            registerLink.classList.add('clicked');
            loginLink.classList.remove('clicked');

            loginForm.classList.add('d-none');
            registerForm.classList.remove('d-none');
        });

        // Add event listener for form submission (you might want to use AJAX)
    ...
    const registerPasswordInput = document.getElementById('register__pass');
    const passwordConditions = document.getElementById('passwordConditions');
    const registerButton = document.querySelector('#login form.d-none button');

    registerPasswordInput.addEventListener('input', function () {
        const password = registerPasswordInput.value;

        // Kiểm tra điều kiện và cập nhật giao diện
        const hasSpecialChar = /[!@#$%^&*(),.?":{}|<>]/.test(password);
        const hasNumber = /\d/.test(password);
        const hasMinLength = password.length >= 8;

        // Kiểm tra điều kiện và thay đổi màu sắc và gạch chân
        document.querySelectorAll('#passwordConditions i')[0].style.color = /[!@#$%^&*(),.?":{}|<>]/.test(password) ? 'green' : 'red';
        ...
        // Cập nhật màu sắc và trạng thái nút Đăng ký
        const passwordConditions = document.getElementById('passwordConditions');
